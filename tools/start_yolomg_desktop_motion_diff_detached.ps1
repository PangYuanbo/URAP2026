param(
    [string]$RunId = "desktop_yolomg_motion_diff_samples",
    [double]$DurationSeconds = 10,
    [int]$ProcessWidth = 1280,
    [int]$DisplayWidth = 960
)
$repoRoot = Split-Path -Parent $PSScriptRoot
$python = Join-Path $repoRoot "URAP-UAV-to-UAV-Detection-and-Tracking\papers\YOLOMG\.venv\Scripts\python.exe"
$script = Join-Path $repoRoot "tools\yolomg_video_motion_diff.py"
$runRoot = Join-Path $repoRoot ("artifacts\detached_yolomg_motion_diff\" + $RunId)
$outputDir = Join-Path $repoRoot ("artifacts\yolomg_desktop_motion_diff\" + $RunId)
$pidPath = Join-Path $runRoot "run.pid"
$stdoutPath = Join-Path $runRoot "stdout.log"
$stderrPath = Join-Path $runRoot "stderr.log"
$progressPath = Join-Path $runRoot "progress.json"
New-Item -ItemType Directory -Force -Path $runRoot, $outputDir | Out-Null
if (Test-Path $pidPath) {
    $oldPid = [int](Get-Content $pidPath -Raw)
    $oldProcess = Get-CimInstance Win32_Process -Filter "ProcessId=$oldPid" -ErrorAction SilentlyContinue
    if ($oldProcess -and $oldProcess.CommandLine -like "*yolomg_video_motion_diff.py*") { throw "A matching job is already running with PID $oldPid" }
}
$videos = Get-ChildItem -LiteralPath ([Environment]::GetFolderPath("Desktop")) -File | Where-Object { $_.Name -like "DJI_*_W.MP4" } | Sort-Object Name | Select-Object -ExpandProperty FullName
if (-not $videos) { throw "No DJI_*_W.MP4 videos found on the desktop." }
$arguments = @($script, "--inputs") + $videos + @("--output-dir", $outputDir, "--duration-seconds", $DurationSeconds, "--process-width", $ProcessWidth, "--display-width", $DisplayWidth, "--progress-json", $progressPath)
$process = Start-Process -FilePath $python -ArgumentList $arguments -WorkingDirectory $repoRoot -WindowStyle Hidden -RedirectStandardOutput $stdoutPath -RedirectStandardError $stderrPath -PassThru
Set-Content -LiteralPath $pidPath -Value $process.Id -Encoding ascii
Set-Content -LiteralPath (Join-Path $runRoot "started_at.txt") -Value (Get-Date).ToString("o") -Encoding ascii
Write-Output "Started detached YOLOMG motion-difference render."
Write-Output "PID=$($process.Id)"
Write-Output "Output=$outputDir"
Write-Output "Progress=$progressPath"
Write-Output "Logs=$stdoutPath ; $stderrPath"