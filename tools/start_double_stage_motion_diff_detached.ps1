param(
    [string]$RunId = "double_stage_dji0619_sample",
...
$script = Join-Path $repoRoot "tools\double_stage_motion_diff.py"
$seaRoot = Join-Path $repoRoot "third_party\SEA-RAFT"
$cfg = Join-Path $seaRoot "config\eval\spring-M.json"
$runRoot = Join-Path $repoRoot ("artifacts\detached_double_stage_diff\" + $RunId)
$outputDir = Join-Path $repoRoot ("artifacts\double_stage_diff\" + $RunId)
...
if (Test-Path $pidPath) {
    ...
    if ($oldProcess -and $oldProcess.CommandLine -like "*double_stage_motion_diff.py*") { throw "A matching job is already running with PID $oldPid" }
}
...
Write-Output "Started detached double-stage motion-difference experiment."
