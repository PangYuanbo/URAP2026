...
RUN=REPO/'artifacts'/'detached_action_chunk_projected_causal_memory_v62'
PROGRESS=RUN/'progress.json'
OUT=Path(r'D:\URAP_vatd_rank_results\action_chunk_projected_causal_memory_v62')
...
PREREQUISITE=PERSISTENT/'test_summary.json'
...
def main() -> int:
    ...
    while not PREREQUISITE.is_file():
        report('waiting_for_projected_persistent_v61',0,prerequisite=str(PREREQUISITE))
        time.sleep(60)
    ...
    summary={'protocol':'strict causal projected-deduplicated Action Chunk 1s/3s memory; future evidence training-only; OOF selection; fixed test','validation_selection':validation,'test_fixed':test,'target_map50':.97,'target_met':test['map50']>=.97}
