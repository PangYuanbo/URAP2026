...
RUN = REPO / "artifacts" / "detached_nps_teacher_motion_token_gate_v16"
PROGRESS = RUN / "progress.json"
OUT = Path(r"D:\URAP_vatd_rank_results\nps_teacher_motion_token_gate_v16")
...
TEACHER = Path(r"D:\URAP_vatd_rank_results\nps_teacher_action_bank_v16")
TOTAL = 3


def wait_for_features() -> None:
    required = [TEACHER / "train_summary.json", ONLINE / "val_summary.json", ONLINE / "test_summary.json"]
    while not all(path.is_file() for path in required):
        report("waiting_for_corrected_action_bank", 0, present=[str(path) for path in required if path.is_file()], missing=[str(path) for path in required if not path.is_file()])
        import time
        time.sleep(30)
...
def main() -> int:
    ...
    wait_for_features()
    execute("train_online_candidate_gate", 0, [
        str(PYTHON), str(REPO / "tools" / "train_action_bank_motion_token_listwise.py"),
        "--train-pkl", str(TRAIN_PKL), "--train-aux-tracklets", str(TEACHER / "train_scores.jsonl"),
        ...
        "--score-field", "teacher_motion_token_gate_score", "--token-encoder", "--epochs", "18", "--frame-batch-size", "192", "--inference-batch-size", "16384", "--hidden", "256", "--lr", "0.0004",
    ])
    execute("select_validation_fusion", 1, [
        ...
        "--score-field", "teacher_motion_token_gate_score", "--per-row-score", "--modes", "replace", "linear-mix", "logit-mix", "geom-mix", "fp-suppress", "tp-boost",
    ...
    execute("evaluate_official_test_fixed", 2, [
        str(PYTHON), str(REPO / "tools" / "sweep_tvd_predictionsgt_score_fusion.py"),
        "--tvd-root", str(TVD), "--predictionsgt-pkl", str(TEST_PKL), "--tracklet-jsonl", str(OUT / "test_gate_scores.jsonl"),
        "--score-field", "teacher_motion_token_gate_score", "--per-row-score", "--modes", str(best["mode"]), "--alphas", str(best["alpha"]),
    ...
    summary = {
        "protocol": "train Clips1-36; select Clips37-40; fixed test Clips41-50",
        "selector": "teacher-forced causal 1s/3s Action Bank with 8 short + 16 long rich motion tokens",
        "validation_best": best,
        "test_fixed": test,
        "target_map50": 0.97,
        "target_met": float(test["map50"]) >= 0.97,
        "validation_candidate_oracle_map50": 0.995,
        "test_candidate_oracle_map50": 0.9920831168831169,
    }
