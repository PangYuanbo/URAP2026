...
def main():
 ...
 tf,tb=load_aux(FULL/"train_forward.jsonl"),load_aux(FULL/"train_backward.jsonl");tn,names=load_neighbor(N/"train_neighbor_scores.jsonl");tx,ty,tg,_,_=dataset_arrays(load_predictionsgt(TRAIN),tf,tb,tn,True,sizes);del tf,tb,tn;gc.collect()
 ...
 keep=hard_rows(tx,ty,tg);binary=(ty[keep]>=.5).astype(np.int32);pos=max(1,int(binary.sum()));neg=len(binary)-pos;weights=np.ones(len(keep),np.float32);report("train_models",1,train_rows=len(tx),hard_rows=len(keep),positives=pos)
