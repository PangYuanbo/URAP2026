...
OUT = REPO / "artifacts/nps_sota_research/nps_vatd_cuda_rank_v3"
RUNNER = REPO / "artifacts/nps_sota_research/nps_vatd_cuda_rank_v3_runner"
PROGRESS = RUNNER / "progress.json"
PYTHON = REPO / "papers/TransVisDrone/.venv/Scripts/python.exe"


def write_progress(stage: str, done: int, total: int = 2) -> None:
    PROGRESS.parent.mkdir(parents=True, exist_ok=True)
    PROGRESS.write_text(json.dumps({"stage": stage, "done": done, "total": total, "updated": datetime.now(timezone.utc).astimezone().isoformat()}, indent=2), encoding="utf-8")
