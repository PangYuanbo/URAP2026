...
if(-not(Test-Path $StatePath)){Write-Host 'status: NOT RUNNING';Write-Host 'done/total: 0/6';exit 0}
...
Write-Host ("status: "+$(if($Process){'RUNNING'}else{'NOT RUNNING'}));Write-Host ("done/total: "+$(if($Progress){"$($Progress.done)/$($Progress.total)"}else{'0/6'}));Write-Host ("stage: "+$(if($Progress){$Progress.stage}else{'starting'}));Write-Host "pid: $($State.pid)";Write-Host "start_time: $($State.start_time)"
