param(
    [string]$RunName = "finetune_base_plus_ard100_short2_frozen_b4_stage1",
...
if (Test-Path -LiteralPath $pidPath) {
    ...
    if ($oldProcess -and $oldProcess.CommandLine -like "*run_sam2_train_with_memory_cap.py*") {
        throw "ARD100 short2 frozen batch-4 training is already running with PID $oldPid"
    }
}
...
$config = "configs/sam2.1_training/sam2.1_hiera_b+_ARD100_short2_frozen_b4_local_stage1.yaml"
...
$metadata = [ordered]@{
    ...
    total_windows = 53450
    batch_size = 4
    expected_batches = 13374
    run_root = "U:\URAP_runs\samurai\finetune_base_plus_ard100_short2_frozen_b4_stage1"
