...
RUN = REPO / "artifacts" / "detached_nps_action_memory_cross_attention_v82"
PROGRESS = RUN / "progress.json"
OUT = Path(r"D:\URAP_vatd_rank_results\nps_action_memory_cross_attention_v82")
...
def main() -> int:
    ...
    execute("train_cross_attention", 0, [
        ...
        "--score-field", "action_memory_cross_attention_score", "--cross-attention",
    ...
    execute("select_guarded_residual", 1, [
        ...
        "--cross-attention-jsonl", str(OUT / "val_scores.jsonl"), "--cross-attention-field", "action_memory_cross_attention_score",
    ...
    execute("evaluate_fixed_test", 2, [
        ...
        "--cross-attention-jsonl", str(OUT / "test_scores.jsonl"), "--cross-attention-field", "action_memory_cross_attention_score",
        "--incumbent-cap", "0.5", "--incumbent-weight", "0.5",
    ...
    summary = {
        ...
        "champion": "cross_attention_v82" if float(test["map50"]) > incumbent_test else "incumbent_v60",
    }
