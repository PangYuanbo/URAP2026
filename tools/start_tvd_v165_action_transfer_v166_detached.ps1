...
$Run = Join-Path $Repo 'artifacts\detached_tvd_v165_action_transfer_v166'
$Python = Join-Path $Repo 'URAP-UAV-to-UAV-Detection-and-Tracking\papers\YOLOMG\.venv\Scripts\python.exe'
$Script = Join-Path $Repo 'tools\run_tvd_v165_action_transfer_v166.py'
...
if (Test-Path $PidFile) {
    ...
    if ($Old -and $Old.CommandLine -like '*run_tvd_v165_action_transfer_v166.py*') { Write-Output "ALREADY RUNNING PID=$OldPid"; exit 0 }
}
$Stdout = Join-Path $Run 'stdout.log'; $Stderr = Join-Path $Run 'stderr.log'
Set-Content -LiteralPath $Stdout -Value ''; Set-Content -LiteralPath $Stderr -Value ''
$Started = Get-Date
...
@{pid=$Process.Id;start_time=$Started.ToString('o');command_line="$Python $Script";stdout_log=$Stdout;stderr_log=$Stderr} | ConvertTo-Json | Set-Content -LiteralPath (Join-Path $Run 'meta.json') -Encoding UTF8
