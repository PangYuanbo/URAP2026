...
def main() -> int:
    ...
    if args.sequence_shard_count < 1:
        parser.error("--sequence-shard-count must be at least 1")
    if not 0 <= args.sequence_shard_index < args.sequence_shard_count:
        parser.error("--sequence-shard-index must be within the shard count")
    ...
    with args.out_jsonl.open("w", encoding="utf-8") as target:
        sequence_items = [
            ...
            if index % args.sequence_shard_count == args.sequence_shard_index
        ]
        for sequence, frames in sequence_items:
            ...
            for sequence_frame_index, (frame_id, image_id, item) in enumerate(ordered_frames):
                ...
                future_entry = None if args.reverse else (frames[sequence_frame_index + future_offset] if sequence_frame_index + future_offset < len(frames) else None)
                future_labels = []
    ...
    summary = {
        ...
        "sequence_shard_count": args.sequence_shard_count,
    }
