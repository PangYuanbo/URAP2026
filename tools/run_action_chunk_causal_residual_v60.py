...
RUN=REPO/'artifacts'/'detached_action_chunk_causal_residual_v60'
PROGRESS=RUN/'progress.json'
OUT=Path(r'D:\URAP_vatd_rank_results\action_chunk_causal_residual_v60')
BASE=Path(r'D:\URAP_vatd_rank_results\action_chunk_causal_v38')
MEMORY=Path(r'D:\URAP_vatd_rank_results\action_chunk_causal_memory_v59')
...
def main() -> int:
    ...
    while not PREREQUISITE.is_file():
        report('waiting_for_strict_causal_memory_v59',0,prerequisite=str(PREREQUISITE))
        time.sleep(60)
    ...
    summary={'protocol':'strict causal Action Chunk bounded dual-memory residual; no future test input; OOF selection; fixed test','validation_selection':validation,'test_fixed':test,'target_map50':.97,'target_met':test['map50']>=.97}
