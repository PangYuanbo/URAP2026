...
if (-not (Test-Path $StatePath)) { Write-Host 'status: NOT RUNNING'; Write-Host 'done/total: 0/100'; exit 0 }
...
if ($Progress) { Write-Host "done/total: $($Progress.done)/$($Progress.total)"; Write-Host "stage: $($Progress.stage)" } else { Write-Host 'done/total: 0/100'; Write-Host 'stage: starting' }
