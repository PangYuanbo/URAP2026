...
if (Test-Path $pidFile) {
  ...
  if ($existingPid) {
    ...
    if ($existing -and $existing.CommandLine -like '*score-vatd-motion-action-tracklets*') {
      Write-Host "Route B VATD motion-action score already running: pid=$existingPid"
...
$argList = @(
  '-m', 'qstr_dronedet.cli', 'score-vatd-motion-action-tracklets',
...
Write-Host 'Started detached Route B VATD motion-action scoring.'
