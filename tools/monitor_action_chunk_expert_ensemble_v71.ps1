...
$Run=Join-Path $Repo 'artifacts\detached_action_chunk_expert_ensemble_v71'
...
if($Progress){"done/total: $($Progress.done)/$($Progress.total)";"stage: $($Progress.stage)";"last_output_timestamp: $((Get-Item $ProgressPath).LastWriteTime)"}else{'done/total: 0/7';'stage: starting'}
