...
REPO=Path(r"C:\Users\aaron\Desktop\URAP");PYTHON=Path(sys.executable);RUN=REPO/"artifacts"/"detached_nps_joint_sweep_v18";PROGRESS=RUN/"progress.json";OUT=Path(r"D:\URAP_vatd_rank_results\nps_joint_action_bank_v18");FIELDS=("joint_score","joint_suppress_score","joint_selected","joint_utility","joint_motion","joint_track_quality")
...
def main():
 ...
 choices=[json.loads((sweeps/f"{f}.json").read_text())["best"]|{"field":f} for f in FIELDS];best=max(choices,key=lambda r:r["map50"]);fixed=OUT/"test_fixed_joint.json"
 ...
 test=json.loads(fixed.read_text())["best"];summary={"protocol":"validation select, fixed test","validation_best":best,"test_fixed":test,"target_map50":.97,"target_met":test["map50"]>=.97};(OUT/"official_joint_summary.json").write_text(json.dumps(summary,indent=2),encoding="utf-8");report("done",len(FIELDS)+1,summary=summary);print(json.dumps(summary,indent=2));return 0
