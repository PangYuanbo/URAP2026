...
RUN = REPO / "artifacts" / "detached_nps_online_motion_token_gate_v14"
PROGRESS = RUN / "progress.json"
OUT = Path(r"D:\URAP_vatd_rank_results\nps_online_motion_token_gate_v14")
...
def main() -> int:
    ...
    execute("train_online_candidate_gate", 0, [
        ...
        "--score-field", "online_motion_token_gate_score", "--token-encoder", "--epochs", "18", "--frame-batch-size", "192", "--inference-batch-size", "16384", "--hidden", "256", "--lr", "0.0004",
    ])
    execute("select_validation_fusion", 1, [
        ...
        "--score-field", "online_motion_token_gate_score", "--per-row-score", "--modes", "replace", "linear-mix", "logit-mix", "geom-mix", "fp-suppress", "tp-boost",
    ...
    execute("evaluate_official_test_fixed", 2, [
        ...
        "--score-field", "online_motion_token_gate_score", "--per-row-score", "--modes", str(best["mode"]), "--alphas", str(best["alpha"]),
    ...
    summary = {
        ...
        "selector": "causal 1s/3s Action Bank with 8 short + 16 long rich motion tokens",
