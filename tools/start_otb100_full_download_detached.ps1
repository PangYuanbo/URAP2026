...
$Stdout=Join-Path $Logs "otb100_full_$Stamp.out.txt"
$Stderr=Join-Path $Logs "otb100_full_$Stamp.err.txt"
$Process=Start-Process -WindowStyle Hidden -FilePath $Python -ArgumentList @(Join-Path $Repo 'tools\download_otb100_github_releases.py') -WorkingDirectory $Repo -RedirectStandardOutput $Stdout -RedirectStandardError $Stderr -PassThru
