param(
  [string]$OutputRoot = 'C:\Users\aaron\Desktop\URAP\artifacts\route_b_official\aot_part0_vatd_motion_action_train_runner',
...
if ($process -and $process.CommandLine -like '*train-vatd-motion-action-policy*') {
  Write-Host ""
...
if ($stdout -and (Test-Path $stdout)) {
  $progressLines = Get-Content $stdout -Tail 2000 | Where-Object {
    $_ -like '*"kind": "vatd_motion_action_train_progress"*' -or
    $_ -like '*"kind": "vatd_motion_action_train_start"*' -or
    $_ -like '*vatd_motion_action_train*'
  }
...
$samplesTotal = 0
if ($progress) {
  if ($progress.kind -eq 'vatd_motion_action_train_progress') {
  ...
  } elseif ($progress.kind -eq 'vatd_motion_action_train_start') {
    $epochs = [int]$progress.epochs
    $batchesTotal = [int]$progress.batches_per_epoch
    $samplesTotal = [int]$progress.samples_total
  } elseif ($progress.vatd_motion_action_train) {
    $epoch = [int]$progress.vatd_motion_action_train.epoch
  }
}
...
if ($out -and (Test-Path $out)) {
  $lastUnit = 'vatd_motion_action_checkpoint'
