param(
  ...
  [string]$RunId = 'aot_action_chunk_transfer_protected_v2'
)
...
$configs = @(
  @{name='b0p22_p0p25'; beta=0.22; protect=0.25},
  @{name='b0p30_p0p25'; beta=0.30; protect=0.25},
  @{name='b0p40_p0p25'; beta=0.40; protect=0.25},
  @{name='b0p30_p0p30'; beta=0.30; protect=0.30}
)
...
$rows=@();$done=1
foreach($config in $configs){$rescore=Join-Path $outRoot "val_$($config.name)";Run-Logged "val_$($config.name)_rescore" $qstrPython @('-m','qstr_dronedet.cli','rescore-aot-prediction-parts-by-tracklets','--results-folder',$part0,'--tracklet-jsonl',$tracklets,'--out-dir',$rescore,'--score-field','vatd_score','--center','0.5','--beta',[string]$config.beta,'--mode','suppress-only','--protect-raw-score-at',[string]$config.protect,'--min-tracklet-rows','1','--missing-score-behavior','keep') $RepoRoot;$metrics=Eval-Aot "val_$($config.name)_eval" (Join-Path $rescore 'aotpredictions') (Join-Path $outRoot "val_$($config.name)_eval");$rows+=[pscustomobject]@{name=$config.name;beta=$config.beta;protect=$config.protect;fppi=$metrics.fppi;hfar=$metrics.hfar;afdr=$metrics.afdr;edr300=$metrics.edr300;summary=$metrics.summary};$done++;Progress 'validation_sweep' $done 7 @{baseline_validation=$baseline;candidates=$rows}}
$eligible=@($rows|Where-Object{$_.edr300 -ge ($baseline.edr300-1e-9) -and $_.hfar -le ($baseline.hfar+1e-6)});if($eligible.Count -eq 0){$eligible=@($rows|Sort-Object @{Expression='edr300';Descending=$true},fppi)};$best=$eligible|Sort-Object fppi,@{Expression='edr300';Descending=$true}|Select-Object -First 1
Progress 'full_rescore' 5 7 @{selected=$best;candidates=$rows}
$fullRescore=Join-Path $outRoot 'full_fixed';Run-Logged 'full_rescore' $qstrPython @('-m','qstr_dronedet.cli','rescore-aot-prediction-parts-by-tracklets','--results-folder',$source,'--tracklet-jsonl',$tracklets,'--out-dir',$fullRescore,'--score-field','vatd_score','--center','0.5','--beta',[string]$best.beta,'--mode','suppress-only','--protect-raw-score-at',[string]$best.protect,'--min-tracklet-rows','1','--missing-score-behavior','keep') $RepoRoot
Progress 'full_official_eval' 6 7 @{selected=$best}
$full=Eval-Aot 'full_official_eval' (Join-Path $fullRescore 'aotpredictions') (Join-Path $outRoot 'full_official_eval');$base=@{fppi=0.262303510022747;hfar=89.47674418604652;afdr=0.8685312193818473;edr300=0.9257142857142857};$summary=@{protocol='part0 validation-selected protected zero-shot Action Chunk transfer; fixed full AOT';baseline_full=$base;validation_baseline=$baseline;validation_candidates=$rows;selected=$best;full_fixed=$full;relative_fppi_reduction=($base.fppi-$full.fppi)/$base.fppi;absolute_afdr_gain=$full.afdr-$base.afdr;absolute_edr300_gain=$full.edr300-$base.edr300;strict_target_met=((($base.fppi-$full.fppi)/$base.fppi)-ge 0.03 -and (($base.fppi-$full.fppi)/$base.fppi)-le 0.05 -and $full.edr300-ge $base.edr300 -and $full.hfar-le $base.hfar)};$summary|ConvertTo-Json -Depth 10|Set-Content (Join-Path $outRoot 'official_summary.json') -Encoding UTF8;Progress 'done' 7 7 @{summary=$summary}
