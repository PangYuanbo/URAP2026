...
RUN = ROOT / "artifacts" / "detached_tvd_oof_stack_v127"
OUT = Path(r"D:\URAP_vatd_rank_results\tvd_oof_stack_v127")
...
def load_final_scores(split: str, data: dict[str, Any], locations: list[tuple[str, int, int, str, float]]) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    for sequence, frame_id, index, image_id, raw in locations:
        ...
        scores119.append(fuse_score(raw, float(v126_map.get(key, raw)), float(v126_config["alpha"]), str(v126_config["mode"])))
...
def main() -> int:
    ...
    for mode in ("logit", "geom", "linear"):
        for weight119 in grid:
            ...
            result = {"mode": mode, "weights": {"v53": weights[0], "v126": weights[1]}, **metrics(correct, blend(val_scores, weights, mode), pred_cls, target_cls, Path(r"D:\urap_modal_stage\TransVisDrone"))}
            rows.append(result)
    ...
    weights = (float(best["weights"]["v53"]), float(best["weights"]["v126"]))
