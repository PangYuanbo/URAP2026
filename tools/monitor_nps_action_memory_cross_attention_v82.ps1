...
$Descendants=@()
$Frontier=@($ChildId)|Where-Object{$_ -gt 0}
while($Frontier.Count){
  $Next=@()
  foreach($ParentId in $Frontier){
    $Found=@($AllProcesses|Where-Object{$_.ParentProcessId -eq $ParentId})
    $Descendants+=$Found
    $Next+=@($Found|ForEach-Object{$_.ProcessId})
  }
  $Frontier=$Next
}
...
foreach($Runtime in $Descendants){Write-Host "runtime_pid: $($Runtime.ProcessId)";Write-Host "runtime_command: $($Runtime.CommandLine)";Write-Host "runtime_working_set_mb: $([math]::Round($Runtime.WorkingSetSize/1MB,1))"}
...
$GpuPids=@($State.pid,$ChildId)+@($Descendants|ForEach-Object{$_.ProcessId})
