param(
  ...
  [string]$ScoreOut = 'artifacts\nps_sota_research\tvd_nps_test_tracklets_v2\ego_adaptive_vatd_scores.jsonl',
  [string]$AttachedOut = 'artifacts\nps_sota_research\tvd_nps_test_tracklets_v2\tracklets_with_ego_adaptive_vatd_scores.jsonl',
  [string]$SweepOutJson = 'artifacts\nps_sota_research\tvd_nps_test_ego_adaptive_vatd_sweep.json',
  [string]$ComparisonOutCsv = 'artifacts\nps_sota_research\tvd_nps_test_ego_adaptive_vatd_sweep_comparison.csv',
  [string]$ComparisonOutJson = 'artifacts\nps_sota_research\tvd_nps_test_ego_adaptive_vatd_sweep_comparison.json',
  [string]$BestPkl = 'artifacts\nps_sota_research\tvd_nps_test_ego_adaptive_vatd_best_predictionsgt.pkl',
...
if (Test-Path $pidFile) {
  ...
  if ($existingPid) {
    ...
    if ($existing -and $existing.CommandLine -like "*$RunId*") {
      Write-Host "NPS Ego-Adaptive VATD score+sweep already running: pid=$existingPid"
...
Write-Host '{"kind":"nps_ego_adaptive_vatd_score_sweep_start","stage":"score"}'
& '$Python' -m qstr_dronedet.cli score-ego-adaptive-vatd-tracklets --tracklet-jsonl '$TrackletJsonl' --weights '$Weights' --out '$ScoreOut' --frame-root '$FrameRoot' --error-scale '$ErrorScale' --min-tracklet-rows '$MinTrackletRows' --batch-size '$ScoreBatchSize' --num-workers '$NumWorkers' --frame-cache-size '$FrameCacheSize' --fusion-mode motion_action
Write-Host '{"kind":"nps_ego_adaptive_vatd_score_sweep_progress","stage":"attach"}'
...
Write-Host '{"kind":"nps_ego_adaptive_vatd_score_sweep_progress","stage":"sweep"}'
...
Write-Host '{"kind":"nps_ego_adaptive_vatd_score_sweep_progress","stage":"claim_gate"}'
...
Write-Host '{"kind":"nps_ego_adaptive_vatd_score_sweep_done","stage":"done","sweep_out":"$SweepOutJson","comparison_json":"$ComparisonOutJson"}'
"@
...
Write-Host 'Started detached NPS Ego-Adaptive VATD score+sweep pipeline.'
