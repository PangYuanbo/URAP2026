...
RUN = ROOT / "artifacts" / "detached_tvd_oof_stack_v125"
OUT = Path(r"D:\URAP_vatd_rank_results\tvd_oof_stack_v125")
...
V123 = Path(r"D:\URAP_vatd_rank_results\tvd_domain_balanced_action_v123")
UPSTREAM_RUN = ROOT / "artifacts" / "detached_tvd_domain_balanced_action_v123"
VATD_MAP50 = 0.93844
...
def load_final_scores(split: str, data: dict[str, Any], locations: list[tuple[str, int, int, str, float]]) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    v123_summary = json.loads((V123 / "official_summary.json").read_text(encoding="utf-8"))
    selected_ratio = float(v123_summary["selected_ratio"])
    selected_record = next(record for record in v123_summary["ratios"] if abs(float(record["ratio"]) - selected_ratio) < 1e-9)
    v123_config = v123_summary["validation_selection"]
    v123_path = Path(selected_record["val_scores"] if split == "val" else selected_record["test_scores"])
    v123_map, _ = load_row_scores(v123_path, str(selected_record["field"]), 1)
    ...
    for sequence, frame_id, index, image_id, raw in locations:
        ...
        scores119.append(fuse_score(raw, float(v123_map.get(key, raw)), float(v123_config["alpha"]), str(v123_config["mode"])))
...
def main() -> int:
    ...
    wait_for(V123 / "official_summary.json", UPSTREAM_RUN)
    ...
    for mode in ("logit", "geom", "linear"):
        for weight119 in grid:
            ...
            result = {"mode": mode, "weights": {"v53": weights[0], "v123": weights[1]}, **metrics(correct, blend(val_scores, weights, mode), pred_cls, target_cls, Path(r"D:\urap_modal_stage\TransVisDrone"))}
            rows.append(result)
    ...
    weights = (float(best["weights"]["v53"]), float(best["weights"]["v123"]))
