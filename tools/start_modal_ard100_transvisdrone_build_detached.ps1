...
$script = Join-Path $RepoRoot "tools\modal_build_ard100_transvisdrone.py"
...
@("started=$((Get-Date).ToString('o'))", "pid=$($process.Id)", "command=modal.exe $($arguments -join ' ')", "stdout=$stdout", "stderr=$stderr") |
