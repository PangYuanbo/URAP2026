...
RUN=ROOT/"artifacts"/"detached_tvd_confidence_band_v139";OUT=Path(r"D:\URAP_vatd_rank_results\tvd_confidence_band_v139");TVD=Path(r"D:\urap_modal_stage\TransVisDrone");VATD=.93844
...
def gated(raw,action,low,high,strength,topk,locations):
 out=raw.copy();mask=(raw>=low)&(raw<=high)
 if topk>0:
  frame={}
  for i,(seq,fid,idx,image_id,r) in enumerate(locations):frame.setdefault(image_id,[]).append(i)
  allowed=np.zeros(len(raw),bool)
  for ids in frame.values():
   order=np.asarray(ids)[np.argsort(raw[ids])[::-1][:topk]];allowed[order]=True
  mask&=allowed
 out[mask]=sigmoid((1-strength)*logit(raw[mask])+strength*logit(action[mask]));return out,int(mask.sum())
def main():
 ...
 lows=(0,.005,.01,.02,.05,.1,.2,.3);highs=(.05,.1,.2,.3,.4,.5,.7,1.0);strengths=(.25,.5,.75,1.0);topks=(0,1,3,5,10)
 for low in lows:
  for high in highs:
   if high<=low:continue
   for strength in strengths:
    for topk in topks:
     score,changed=gated(raw,action,low,high,strength,topk,loc);row={"low":low,"high":high,"strength":strength,"topk":topk,"changed_rows":changed,**metrics(correct,score,pred,target,TVD)};rows.append(row)
 best=max(rows,key=lambda r:float(r["map50"]));(OUT/"val_sweep.json").write_text(json.dumps({"best":best,"top":sorted(rows,key=lambda r:-float(r["map50"]))[:50],"labels":labels},indent=2));report("fixed_test",2,validation_selection=best);q=load_predictionsgt(TEST);qc,qp,qt,qloc,qlabels=flat_stats(q);qraw=np.asarray([x[4] for x in qloc],np.float64);qaction=v130_scores("test",q,qloc);score,changed=gated(qraw,qaction,float(best["low"]),float(best["high"]),float(best["strength"]),int(best["topk"]),qloc);test={**metrics(qc,score,qp,qt,TVD),"changed_rows":changed,"labels":qlabels,"detections":len(qloc)};gain=100*(test["map50"]-VATD);summary={"protocol":"validation-selected raw-confidence-band routing of V130; fixed test","validation_selection":best,"test_fixed":test,"vatd_map50":VATD,"gain_over_vatd_points":gain,"target_3_to_5_met":3<=gain<=5};(OUT/"official_summary.json").write_text(json.dumps(summary,indent=2));report("done",3,summary=summary);return 0
