...
def main() -> int:
    ...
    parser.add_argument("--teacher-force-updates", action="store_true")
            ...
            for sequence_frame_index, (frame_id, image_id, item) in enumerate(frames):
                ...
                for candidate_index, detection in enumerate(detections):
                    ...
                    rows.append({
                        ...
                        "online_action_bank_short_tokens": active[best_track_index].candidate_motion_tokens(candidate, timestamp, transforms[best_track_index], args.short_seconds, args.short_token_count, finite(detection.get("score"))) if best_track_index is not None else [0.0] * (12 * args.short_token_count),
                        "online_action_bank_long_tokens": active[best_track_index].candidate_motion_tokens(candidate, timestamp, transforms[best_track_index], args.long_seconds, args.long_token_count, finite(detection.get("score"))) if best_track_index is not None else [0.0] * (12 * args.long_token_count),
                ...
                if args.teacher_force_updates:
                    teacher_detections = [
                        {"bbox": list(label["bbox"]), "score": 1.0}
                        for label in item.get("labels", [])
                        if isinstance(label.get("bbox"), list) and len(label["bbox"]) == 4
                    ]
                    teacher_scores = [
                        [track.score_candidate(box(label), timestamp, transforms[index], validities[index], args.short_seconds, args.long_seconds) for index, track in enumerate(active)]
                        for label in teacher_detections
                    ]
                    proposals = choose_updates(active, teacher_detections, teacher_scores, transforms, frame_id, timestamp, args.internal_alpha, 0.0)
                    proposals.extend(OnlineActionTrack(frame_id, timestamp, box(label), 1.0) for label in teacher_detections)
                else:
    ...
    summary = {
        ...
        "frames_with_active_bank": frames_with_tracks, "teacher_force_updates": args.teacher_force_updates, "sequences": summaries,
    }
