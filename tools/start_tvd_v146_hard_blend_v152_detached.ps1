...
$runDir = Join-Path $repo 'artifacts\detached_tvd_v146_hard_blend_v152'
$logDir = Join-Path $runDir 'logs'
New-Item -ItemType Directory -Force -Path $logDir | Out-Null
$timestamp = Get-Date -Format 'yyyyMMdd_HHmmss'
$stdoutLog = Join-Path $logDir "v152_$timestamp.out.txt"
$stderrLog = Join-Path $logDir "v152_$timestamp.err.txt"
$python = 'C:\Users\aaron\Desktop\URAP\URAP-UAV-to-UAV-Detection-and-Tracking\papers\YOLOMG\.venv\Scripts\python.exe'
$process = Start-Process -FilePath $python -ArgumentList @('tools\run_tvd_v146_hard_blend_v152.py') -WorkingDirectory $repo -PassThru -WindowStyle Hidden -RedirectStandardOutput $stdoutLog -RedirectStandardError $stderrLog
