...
$process=Start-Process -FilePath 'C:\Program Files\dotnet\dotnet.exe' -ArgumentList @('C:\Users\aaron\Desktop\URAP\tmp\pwsh\.store\powershell\7.4.6\powershell\7.4.6\tools\net8.0\any\win\pwsh.dll','-NoProfile','-File',(Join-Path $RepoRoot 'tools\download_nps_test_frames_archive.ps1')) -WorkingDirectory $RepoRoot -RedirectStandardOutput $stdout -RedirectStandardError $stderr -WindowStyle Hidden -PassThru
Set-Content (Join-Path $root ($RunId+'.pid')) $process.Id
@{pid=$process.Id;started=(Get-Date -Format o);stdout=$stdout;stderr=$stderr;archive='D:\URAP_nps_test_pack\NPS_AllFrames_test.tar'}|ConvertTo-Json|Set-Content (Join-Path $root ($RunId+'.meta.json'))
