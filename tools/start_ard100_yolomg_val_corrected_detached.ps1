...
$name = 'yolomg_ard100_val_candidates'

New-Item -ItemType Directory -Force -Path $logDir | Out-Null
if (Test-Path $pidFile) {
    $oldPid = [int](Get-Content $pidFile -Raw)
    if (Get-Process -Id $oldPid -ErrorAction SilentlyContinue) {
        throw "Corrected YOLOMG ARD100 val run is active with PID $oldPid"
    }
}
$timestamp = Get-Date -Format 'yyyyMMdd_HHmmss'
$stdoutLog = Join-Path $logDir "val_corrected_$timestamp.out.txt"
$stderrLog = Join-Path $logDir "val_corrected_$timestamp.err.txt"
$python = Join-Path $repo '.venv\Scripts\python.exe'
$arguments = @(
    'val.py', '--data', $data, '--weights', $weights,
    '--task', 'val', '--task2', 'val2', '--imgsz', "$ImgSz",
...
@{
    ...
    weights = $weights
    total_images = 20762
    total_batches = [math]::Ceiling(20762 / $BatchSize)
