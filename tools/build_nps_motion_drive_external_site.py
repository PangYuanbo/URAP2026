...
<head>
  ...
  <title>NPS Optical-Flow Motion Boundary Gallery - Drive Links</title>
  <style>
    :root { color-scheme: dark; --bg:#101214; --panel:#191d20; --panel2:#22282c; --text:#edf1f4; --muted:#9aa6ad; --line:#343c42; --accent:#27d35f; --good:#86efac; }
    * { box-sizing: border-box; }
    ...
    @media (max-width: 900px) { body { overflow:auto; } .app { grid-template-columns: 1fr; height:auto; } .list { max-height: 420px; } .topbar, .footer { flex-direction: column; align-items: flex-start; } iframe { height: 56vh; } }
  </style>
</head>
<body>
  <div class="app">
    ...
    <main>
      ...
      <div class="stage"><iframe id="player" allow="autoplay; fullscreen" allowfullscreen></iframe></div>
      <div class="footer">
        <span id="path"></span>
  ...
  <script>
    ...
    function renderList() {
      ...
      state.filtered.forEach((v, idx) => {
        ...
        btn.innerHTML = `<img class="thumb" src="${v.poster || ''}" alt=""><span><span class="name">${v.video}</span><span class="mini">${v.split} | ${v.frames} frames</span><span class="mini">${v.method}</span><span class="mini ready">Drive linked</span></span>`;
    ...
    fetch('videos_external.json').then(r => r.json()).then(videos => {
      state.videos = videos;
      ...
      renderList();
      selectVideo(0);
    }).catch(err => { els.title.textContent = 'Failed to load videos_external.json'; els.meta.textContent = String(err); });
  </script>
</body>
</html>
"""
