...
def create_tvl1():
    if cv2.cuda.getCudaEnabledDeviceCount() < 1:
        raise RuntimeError("No CUDA device is available to OpenCV.")
    return cv2.cuda.OpticalFlowDual_TVL1_create()


def calc_gpu_flow(engine, source_gray, reference_gray):
    source_gpu = cv2.cuda_GpuMat()
    reference_gpu = cv2.cuda_GpuMat()
    source_gpu.upload(source_gray)
    reference_gpu.upload(reference_gray)
    flow_gpu = engine.calc(source_gpu, reference_gpu, None)
...
def main():
    ...
    while done < output_frames:
        ...
        previous_flow = calc_gpu_flow(previous_engine, previous_gray, reference_gray)
        following_flow = calc_gpu_flow(following_engine, following_gray, reference_gray)
    ...
    manifest.write_text(json.dumps({"method": "NPS paper CUDA Dual TV-L1 dense flow head and motion boundary; YOLOMG RANSAC homography, warp, and bidirectional compensated difference tail", "input": str(args.input), "output": str(output_path), "thumbnail": str(thumbnail_path), "frames": done, "fps": fps}, indent=2), encoding="utf-8")
