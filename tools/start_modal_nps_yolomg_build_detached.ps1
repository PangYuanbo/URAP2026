param(
    ...
    [string]$RunnerDir = "artifacts\modal_nps_yolomg_build"
)
...
if ($oldPid -match "^\d+$" -and (Get-Process -Id ([int]$oldPid) -ErrorAction SilentlyContinue)) {
    throw "Modal NPS YOLOMG build already running PID=$oldPid"
}
$stamp = Get-Date -Format "yyyyMMdd_HHmmss"
$stdout = Join-Path $LogDir "modal_nps_yolomg_build_$stamp.out.txt"
$stderr = Join-Path $LogDir "modal_nps_yolomg_build_$stamp.err.txt"
$script = Join-Path $RepoRoot "tools\modal_build_nps_yolomg.py"
$arguments = @("run", $script, "--splits", "train")
...
@(
    ...
    "stdout=$stdout"
    "stderr=$stderr"
) | Set-Content -LiteralPath (Join-Path $RunnerDir "modal_nps_yolomg_build.meta.txt") -Encoding utf8
Get-Content (Join-Path $RunnerDir "modal_nps_yolomg_build.meta.txt")
