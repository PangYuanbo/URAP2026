...
RUN = REPO / "artifacts" / "detached_nps_action_bank_native_token_gate_v11"
PROGRESS = RUN / "progress.json"
OUT = Path(r"D:\URAP_vatd_rank_results\nps_action_bank_native_token_gate_v11")
...
def wait_for_features() -> None:
    required = [
        ...
        NATIVE_TEST / "test_score_summary.json",
    ]
    while not all(path.is_file() for path in required):
        ...
        time.sleep(60)
    report("feature_inputs_ready", 0, inputs=[str(path) for path in required])
...
def main() -> int:
    ...
    wait_for_features()
    execute("train_candidate_gate", 0, [
        ...
        "--score-field", "action_bank_native_token_gate_score", "--token-encoder", "--epochs", "18", "--frame-batch-size", "192", "--inference-batch-size", "16384", "--hidden", "256", "--lr", "0.0004",
    ])
    execute("select_validation_fusion", 1, [
        ...
        "--score-field", "action_bank_native_token_gate_score", "--per-row-score", "--modes", "replace", "linear-mix", "logit-mix", "geom-mix", "fp-suppress", "tp-boost",
    ...
    execute("evaluate_official_test_fixed", 2, [
        ...
        "--score-field", "action_bank_native_token_gate_score", "--per-row-score", "--modes", str(best["mode"]), "--alphas", str(best["alpha"]),
    ...
    summary = {"protocol": "train Clips1-36; select Clips37-40; fixed test Clips41-50", "validation_best": best, "test_fixed": test, "target_map50": 0.97, "target_met": float(test["map50"]) >= 0.97, "future_supervision": "future 1-second consistency used only in training", "action_tokens": "8 causal short-term tokens over 1 second + 16 causal long-term tokens over 3 seconds"}
