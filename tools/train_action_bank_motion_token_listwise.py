...
ACTION_QUERY_NAMES = [
    ...
    "samurai_cmc_forward_iou", "samurai_cmc_residual_speed", "samurai_cmc_camera_validity", "camera_motion_validity",
    "online_action_bank_score", "online_action_bank_predicted_iou", "online_action_bank_center_similarity",
    "online_action_bank_direction_similarity", "online_action_bank_scale_similarity", "online_action_bank_track_quality",
    "online_action_bank_track_age_seconds", "online_action_bank_acceleration_similarity",
    "online_action_bank_motion_stability", "online_action_bank_hypotheses",
]
NATIVE_NAMES = [
...
def group_loss(logits: torch.Tensor, ious: torch.Tensor, future_consistency: torch.Tensor, groups: list[tuple[int, int]], device: torch.device, robust_pairwise: bool = False) -> torch.Tensor:
    ...
    for start, stop in groups:
        ...
        if positives.any():
            ...
            if robust_pairwise:
                positive_score = scores[positives].mean()
                negative_scores = scores[~positives]
                negative_score = torch.logsumexp(negative_scores, dim=0) - math.log(max(1, negative_scores.numel())) if negative_scores.numel() else positive_score.detach()
                pairwise = torch.nn.functional.softplus(-(positive_score - negative_score))
            else:
...
def cap_training_candidates(features: np.ndarray, ious: np.ndarray, future: np.ndarray, groups: list[tuple[int, int]], maximum: int) -> tuple[np.ndarray, np.ndarray, np.ndarray, list[tuple[int, int]]]:
    if maximum <= 0:
        return features, ious, future, groups
    raw_index = FEATURE_NAMES.index("raw_score")
    feature_chunks, iou_chunks, future_chunks, capped_groups = [], [], [], []
    cursor = 0
    for start, stop in groups:
        count = stop - start
        local = np.arange(count, dtype=np.int64)
        if count > maximum:
            local_iou = ious[start:stop]
            positives = local[local_iou >= 0.5]
            negatives = local[local_iou < 0.5]
            negative_budget = max(0, maximum - len(positives))
            if len(negatives) > negative_budget:
                negative_scores = features[start:stop, raw_index][negatives]
                negatives = negatives[np.argsort(-negative_scores, kind="stable")[:negative_budget]]
            local = np.sort(np.concatenate((positives, negatives)))
        feature_chunks.append(features[start:stop][local])
        iou_chunks.append(ious[start:stop][local])
        future_chunks.append(future[start:stop][local])
        capped_groups.append((cursor, cursor + len(local)))
        cursor += len(local)
    return np.concatenate(feature_chunks), np.concatenate(iou_chunks), np.concatenate(future_chunks), capped_groups
...
def main() -> int:
    ...
    parser.add_argument("--write-loss-weight", type=float, default=0.05)
    parser.add_argument("--action-only-query", action="store_true")
    parser.add_argument("--init-model", type=Path)
    parser.add_argument("--max-train-candidates-per-frame", type=int, default=0)
    parser.add_argument("--robust-pairwise", action="store_true")
    ...
    uncapped_train_rows = len(train_x)
    train_x, train_iou, train_future, train_groups = cap_training_candidates(
        train_x, train_iou, train_future, train_groups, args.max_train_candidates_per_frame
    )
    ...
    initial_checkpoint = torch.load(args.init_model, map_location="cpu", weights_only=False) if args.init_model else None
    if initial_checkpoint is not None:
        if list(initial_checkpoint.get("features") or []) != FEATURE_NAMES:
            raise ValueError("initial checkpoint feature schema does not match")
        mean = np.asarray(initial_checkpoint["mean"], dtype=np.float32)
        std = np.asarray(initial_checkpoint["std"], dtype=np.float32)
    else:
        mean = train_x.mean(axis=0)
        std = train_x.std(axis=0)
    std[std < 1e-6] = 1.0
    if args.token_encoder or args.cross_attention:
    ...
    if args.cross_attention:
        model = ActionMemoryCrossAttentionRanker(
            ...
            query_indices=[FEATURE_NAMES.index(name) for name in ACTION_QUERY_NAMES] if args.action_only_query else None,
            use_static_head=not args.action_only_query,
        ).to(device)
    elif args.token_encoder:
        model = ActionTokenRanker(normalized.shape[1], hidden=args.hidden).to(device)
    else:
        model = MLP(normalized.shape[1], hidden=args.hidden).to(device)
    if initial_checkpoint is not None:
        expected_type = "action_memory_cross_attention_action_only" if args.cross_attention and args.action_only_query else ("action_memory_cross_attention" if args.cross_attention else ("action_token_ranker" if args.token_encoder else "mlp"))
        if initial_checkpoint.get("model_type") != expected_type:
            raise ValueError(f"initial checkpoint type {initial_checkpoint.get('model_type')!r} does not match {expected_type!r}")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch_start in range(0, len(group_order), args.frame_batch_size):
            ...
            if args.cross_attention:
                details = model.forward_with_details(batch_x)
                logits = details.logits
                ranking_loss = group_loss(logits, batch_y, batch_future, local_groups, device, args.robust_pairwise)
                write_target = (batch_y >= 0.5).to(details.write_probability.dtype)
                positives = torch.clamp(write_target.sum(), min=1.0)
                positive_weight = torch.clamp((write_target.numel() - positives) / positives, min=1.0, max=32.0)
                write_weights = torch.where(write_target > 0.5, positive_weight, torch.ones_like(write_target))
                write_loss = torch.nn.functional.binary_cross_entropy(
                    details.write_probability, write_target, weight=write_weights
                )
                loss = ranking_loss + args.write_loss_weight * write_loss
                write_losses.append(float(write_loss.detach().cpu()))
            else:
                logits = model(batch_x)
                loss = group_loss(logits, batch_y, batch_future, local_groups, device, args.robust_pairwise)
        ...
        if write_losses:
            row["write_loss"] = float(np.mean(write_losses))
        if device.type == "cuda":
    ...
    model_type = "action_memory_cross_attention_action_only" if args.cross_attention and args.action_only_query else ("action_memory_cross_attention" if args.cross_attention else ("action_token_ranker" if args.token_encoder else "mlp"))
    torch.save({"model": model.state_dict(), "mean": mean, "std": std, "features": FEATURE_NAMES, "hidden": args.hidden, "score_field": args.score_field, "model_type": model_type, "attention_heads": args.attention_heads, "memory_layers": args.memory_layers, "action_only_query": args.action_only_query, "action_query_names": ACTION_QUERY_NAMES if args.action_only_query else None}, args.out_model)
    summary = {
        "device": str(device),
        "model_type": model_type,
        "initialized_from": str(args.init_model) if args.init_model else None,
        "action_tokens": {"short_seconds": 1.0, "short_count": SHORT_TOKEN_COUNT, "long_seconds": 3.0, "long_count": LONG_TOKEN_COUNT, "fields": TOKEN_FIELDS} if args.token_encoder or args.cross_attention else None,
        "memory_attention": {"query": "current Action state only" if args.action_only_query else "current candidate/static state", "key_value": "causal camera-compensated Action tokens", "short_bank_seconds": 1.0, "long_bank_seconds": 3.0, "heads": args.attention_heads, "memory_layers": args.memory_layers, "static_detector_shortcut": not args.action_only_query, "write_probability_head": True, "write_supervision": "greedy IoU>=0.5 reliable candidate", "write_loss_weight": args.write_loss_weight} if args.cross_attention else None,
        "train_rows": len(train_x),
        "uncapped_train_rows": uncapped_train_rows,
        "max_train_candidates_per_frame": args.max_train_candidates_per_frame,
        "robust_pairwise": args.robust_pairwise,
