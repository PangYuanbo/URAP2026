...
$runDir = 'C:\Users\aaron\Desktop\URAP\artifacts\detached_tvd_track_persistence_v155'
$summaryPath = 'D:\URAP_vatd_rank_results\tvd_track_persistence_v155\official_summary.json'
...
if (Test-Path $progressPath) { $progress=Get-Content $progressPath -Raw|ConvertFrom-Json; Write-Host "Progress: $($progress.done)/$($progress.total)"; Write-Host "Stage: $($progress.stage)"; Write-Host "Last output: $($progress.updated)" } else { Write-Host 'Progress: 0/3'; Write-Host 'Last output: no progress file' }
