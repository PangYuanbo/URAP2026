param(
  ...
  [string]$Out = 'artifacts\route_b_official\aot_part0_video_action_train\video_action_policy.pt',
  [int]$PastLen = 3,
  [int]$FutureLen = 2,
  [int]$CropSize = 48,
...
if (Test-Path $pidFile) {
  ...
  if ($existingPid) {
    ...
    if ($existing -and $existing.CommandLine -like '*train-video-action-chunk-policy*') {
      Write-Host "Route B video-action train already running: pid=$existingPid"
...
$argList = @(
  '-m', 'qstr_dronedet.cli', 'train-video-action-chunk-policy',
...
Write-Host 'Started detached Route B video-action training.'
