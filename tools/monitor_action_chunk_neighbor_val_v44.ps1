...
$Run=Join-Path $Repo 'artifacts\detached_action_chunk_neighbor_val_v44'
$StatePath=Join-Path $Run 'state.json'
if(-not(Test-Path $StatePath)){Write-Host 'status: NOT RUNNING';Write-Host 'done/total: 0/1';exit 0}
...
Write-Host ("done/total: "+$(if($Progress){"$($Progress.done)/$($Progress.total)"}else{'0/1'}))
