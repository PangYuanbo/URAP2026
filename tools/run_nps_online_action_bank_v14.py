...
RUN = REPO / "artifacts" / "detached_nps_online_action_bank_v14"
...
def execute(name: str, predictions: Path, frames: Path, homographies: Path, output: Path, summary: Path, done: int) -> None:
    command = [
        ...
        "--homography-cache", str(homographies), "--out-jsonl", str(output), "--out-summary", str(summary),
        "--sequence-fps-json", str(FPS), "--short-seconds", "1.0", "--long-seconds", "3.0",
        "--beam-size", "6", "--short-token-count", "8", "--long-token-count", "16", "--start-gate", "0.12", "--update-gate", "0.08", "--internal-alpha", "2.5",
    ]
...
def wait_for_v13_training() -> None:
    prerequisite = Path(r"D:\URAP_vatd_rank_results\nps_online_token_gate_v13\official_summary.json")
    while not prerequisite.is_file():
        report("waiting_for_v13_fixed_test", 0, prerequisite=str(prerequisite))
        import time
        time.sleep(30)
...
def main() -> int:
    ...
    wait_for_v13_training()
    execute("train_action_bank", Path(r"D:\URAP_nps_train_tvd\runs\nps_train_rank_source\predictionsgt\predictionsgt_split_0.pkl"), Path(r"U:\URAP_datasets\TransVisDrone\NPS\AllFrames\train"), Path(r"D:\URAP_vatd_rank_results\nps_action_bank_cmc_v2\homographies\train.pkl"), OUT / "train_scores.jsonl", OUT / "train_summary.json", 0)
    execute("val_action_bank", Path(r"D:\URAP_nps_val_tvd\runs\nps_val_rank_source\predictionsgt\predictionsgt_split_0_official_labels.pkl"), Path(r"U:\URAP_datasets\TransVisDrone\NPS\AllFrames\val"), Path(r"D:\URAP_vatd_rank_results\nps_action_bank_cmc_v2\homographies\val.pkl"), OUT / "val_scores.jsonl", OUT / "val_summary.json", 1)
    execute("test_action_bank", Path(r"D:\URAP_vatd_rank_inputs\nps_predictionsgt_split_0.pkl"), Path(r"U:\URAP_datasets\TransVisDrone\NPS\AllFrames\test"), Path(r"D:\URAP_vatd_rank_results\nps_action_bank_cmc_v2\homographies\test.pkl"), OUT / "test_scores.jsonl", OUT / "test_summary.json", 2)
