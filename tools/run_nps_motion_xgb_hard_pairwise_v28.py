...
RUN = REPO / "artifacts" / "detached_nps_motion_xgb_hard_pairwise_v28"
PROGRESS = RUN / "progress.json"
OUT = Path(r"D:\URAP_vatd_rank_results\nps_motion_xgb_hard_pairwise_v28")
...
def main() -> int:
    ...
    execute("train_pairwise", 0, [
        ...
        "--score-field", "xgb_hard_pairwise_score", "--hard-mining", "--hard-margin", "0.10",
    ])
    common = [str(PYTHON), str(REPO / "tools" / "sweep_tvd_predictionsgt_score_fusion.py"), "--tvd-root", r"D:\urap_modal_stage\TransVisDrone", "--per-row-score", "--score-field", "xgb_hard_pairwise_score"]
