...
def prepare_frames(source: Path, target: Path, start: int, count: int) -> None:
    ...
    for offset in range(count):
        source_path = source / f"{start + offset:04d}.jpg"
        if not source_path.is_file():
            source_path = source / f"{start + offset:08d}.jpg"
        if not source_path.is_file():
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Evaluate SAMURAI with causal Action Bank memory re-prompting on OTB100.")
    ...
    parser.add_argument("--reprompt-patience", type=int, default=2)
    parser.add_argument("--reprompt-native-quality", type=float, default=0.15)
    parser.add_argument("--reprompt-motion-score", type=float, default=0.38)
    ...
    for sequence_index, (name, attributes) in enumerate(selected, start=1):
        ...
        try:
            with torch.inference_mode(), torch.autocast("cuda", dtype=torch.bfloat16):
                ...
                def adaptive_propagation():
                    next_frame = 0
                    while next_frame < len(groundtruth):
                        restarted = False
                        for output in predictor.propagate_in_video(state, start_frame_idx=next_frame):
                            yield output
                            next_frame = int(output[0]) + 1
                ...
                propagation = adaptive_propagation() if args.reprompt else predictor.propagate_in_video(state)
                for frame_index, object_ids, masks in propagation:
                    ...
                    reprompt_needed = args.reprompt and (raw_box is None or native_quality < args.reprompt_native_quality or (motion is not None and motion.score < args.reprompt_motion_score))
                    low_quality_frames = low_quality_frames + 1 if reprompt_needed else 0
                    corrected_box = None
                    if low_quality_frames >= args.reprompt_patience:
                        _corrected_frame, corrected_ids, corrected_masks = predictor.add_new_points_or_box(state, frame_idx=frame_index, obj_id=0, box=list(predicted_box))
                        if len(corrected_ids) == 1 and len(corrected_masks) == 1:
                            corrected_prediction = mask_box(corrected_masks[0])
                            if valid_box(corrected_prediction):
                                corrected_box = xywh_to_xyxy(corrected_prediction)
                        low_quality_frames = 0
                        reprompt_events += 1
                        restart_requested[0] = True
                    if corrected_box is not None:
                        corrected_motion = bank.score_candidate(corrected_box, timestamp, camera_transform, camera_validity, args.short_seconds, args.long_seconds)
                        output_box = corrected_box
                        bank.update(frame_index, timestamp, corrected_box, max(native_quality, 0.5), corrected_motion.score, camera_transform, args.long_seconds)
        ...
        metrics = {"sequence": name, **sequence_metrics(predictions, groundtruth), "raw_samurai": raw_metrics, "reprompt_events": reprompt_events}
    ...
    summary.update({"benchmark": "OTB100" if benchmark_complete else "OTB100 partial/smoke", "tracker": "SAMURAI + optical-flow CMC + causal 1s/3s Action Bank memory re-prompt", "checkpoint": str(args.checkpoint), "action_bank": {"short_seconds": args.short_seconds, "long_seconds": args.long_seconds, "fps": args.fps, "min_native_quality": args.bank_min_native_quality, "min_motion_score": args.bank_min_motion_score, "blend_weight": args.bank_blend_weight, "camera_compensation": "background LK optical flow + RANSAC homography", "camera_max_size": args.camera_max_size, "reprompt": args.reprompt, "reprompt_patience": args.reprompt_patience, "reprompt_native_quality": args.reprompt_native_quality, "reprompt_motion_score": args.reprompt_motion_score}, "results": results})
