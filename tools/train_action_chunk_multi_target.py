...
def selected_rows(x,y,groups,empty_cap=2,margin=.3,maxneg=24):
 keep=[];weights=[]
 for st,sp in groups:
  positives=np.flatnonzero(y[st:sp]>=.5);negatives=np.flatnonzero(y[st:sp]<.5);raw=x[st:sp,0]
  if not len(negatives):chosen=positives
  elif not len(positives):chosen=negatives[np.argsort(raw[negatives])[::-1][:empty_cap]]
  else:
...
def main():
 p=argparse.ArgumentParser(description='Multi-target-aware pure Action Chunk Bank with moderate empty-frame negatives.')
 ...
 p.add_argument('--score-field',default='action_chunk_multi_target_score');a=p.parse_args();ta,tb=load_aux(a.train_forward),load_aux(a.train_backward);tn,names=load_neighbor(a.train_neighbor);tx,ty,tg,_,_=dataset_arrays(load_predictionsgt(a.train_pkl),ta,tb,tn,True);del ta,tb,tn;gc.collect();va,vb=load_aux(a.val_forward),load_aux(a.val_backward);vn,vnames=load_neighbor(a.val_neighbor);assert names==vnames;vx,vy,vg,vloc,vseq=dataset_arrays(load_predictionsgt(a.val_pkl),va,vb,vn,True);del va,vb,vn;gc.collect();qa,qb=load_aux(a.test_forward),load_aux(a.test_backward);qn,qnames=load_neighbor(a.test_neighbor);assert names==qnames;qx,_,_,qloc,_=dataset_arrays(load_predictionsgt(a.test_pkl),qa,qb,qn,False);del qa,qb,qn;gc.collect();oof=np.zeros(len(vx),np.float32);tests=[];models=[];a.out_model_dir.mkdir(parents=True,exist_ok=True)
 for held in sorted(set(vseq)):
  ...
  dx=np.concatenate(parts);dy=np.concatenate(labels);model,count,pos,pw=fit(dx,dy,groups);mask=vseq==held;oof[mask]=model.predict_proba(vx[mask])[:,1];tests.append(model.predict_proba(qx)[:,1]);path=a.out_model_dir/f'action_chunk_multi_target_without_{held}.ubj';model.save_model(path);record={'excluded_validation_video':held,'rows':count,'positive_rows':pos,'mean_positive_weight':pw,'model':str(path)};models.append(record);print(json.dumps({'kind':'action_chunk_multi_target_model',**record}),flush=True);del dx,dy,model;gc.collect()
 write_score_jsonl(a.out_val_scores,oof,vloc,a.score_field);write_score_jsonl(a.out_test_scores,np.mean(np.stack(tests),axis=0).astype(np.float32),qloc,a.score_field);summary={'model':'multi-target-aware pure Action Chunk Bank','empty_negative_cap':2,'models':models,'features':tx.shape[1]};a.out_summary.write_text(json.dumps(summary,indent=2),encoding='utf8');print(json.dumps(summary,indent=2));return 0
