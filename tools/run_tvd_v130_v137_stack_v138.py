...
RUN = ROOT / "artifacts" / "detached_tvd_v130_v137_stack_v138"
OUT = Path(r"D:\URAP_vatd_rank_results\tvd_v130_v137_stack_v138")
V130 = Path(r"D:\URAP_vatd_rank_results\tvd_oof_stack_v130")
V137 = Path(r"D:\URAP_vatd_rank_results\tvd_cmc_action_v137")
...
def route_scores(split: str, data: dict[str, Any], locations: list[tuple[str, int, int, str, float]]) -> tuple[np.ndarray, np.ndarray]:
    ...
    v137 = json.loads((V137 / "official_summary.json").read_text(encoding="utf-8"))["validation_selection"]
    score_path = V137 / ("val_scores.jsonl" if split == "val" else "test_scores.jsonl")
    score_map, _ = load_row_scores(score_path, "tvd_cmc_action_score", 1)
    cmc_route = np.asarray([
        fuse_score(raw, float(score_map.get((sequence, frame_id, index), raw)), float(v137["alpha"]), str(v137["mode"]))
        for sequence, frame_id, index, _image_id, raw in locations
    ], dtype=np.float64)
    return best_stack, cmc_route
...
def main() -> int:
    ...
    for mode in ("logit", "geom", "linear"):
        for step in range(101):
            weight137 = step / 100.0
            weights = (1.0 - weight137, weight137)
            result = metrics(correct, blend(list(routes), weights, mode), pred_cls, target_cls, TVD_ROOT)
            rows.append({"mode": mode, "weights": {"v130": weights[0], "v137": weights[1]}, **result})
    ...
    weights = (float(best["weights"]["v130"]), float(best["weights"]["v137"]))
    ...
    summary = {"protocol": "validation-selected blend of V130 OOF stack and CMC V137; fixed test", "validation_selection": best, "test_fixed": {**test_result, "labels": test_labels, "detections": len(test_locations)}, "vatd_map50": VATD_MAP50, "gain_over_vatd_points": gain, "target_3_to_5_met": 3.0 <= gain <= 5.0}
