param([string]$RepoRoot='C:\Users\aaron\Desktop\URAP',[string]$RunId='nps_official_val_xgb_rank_v1')
$ErrorActionPreference='Stop'
$root=Join-Path $RepoRoot 'artifacts\detached_nps_official_val_xgb_rank'
...
$script=Join-Path $RepoRoot 'tools\run_nps_official_val_xgb_rank.py'
...
@{pid=$process.Id;started=(Get-Date -Format o);stdout=$stdout;stderr=$stderr;progress=(Join-Path $root 'progress.json')}|ConvertTo-Json|Set-Content (Join-Path $root ($RunId+'.meta.json'))
