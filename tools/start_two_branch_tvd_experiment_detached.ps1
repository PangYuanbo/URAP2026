param(
  [string]$RunName = "two_branch_motion_action_tvd_20260606",
  [int]$Epochs = 20,
  [string]$MaxWindowsPerTracklet = ""
)
...
$Args = @(
  "tools\run_two_branch_tvd_experiment.py",
  "--repo-root", $Repo,
  "--out-dir", $RunDir,
  "--epochs", "$Epochs"
)
if ($MaxWindowsPerTracklet -ne "") {
  $Args += @("--max-windows-per-tracklet", $MaxWindowsPerTracklet)
}
