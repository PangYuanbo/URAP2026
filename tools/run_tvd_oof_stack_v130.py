...
RUN = ROOT / "artifacts" / "detached_tvd_oof_stack_v130"
OUT = Path(r"D:\URAP_vatd_rank_results\tvd_oof_stack_v130")
...
V126 = Path(r"D:\URAP_vatd_rank_results\tvd_domain_balanced_action_v126")
V129 = Path(r"D:\URAP_vatd_rank_results\tvd_domain_balanced_action_v129")
UPSTREAM_RUN = ROOT / "artifacts" / "detached_tvd_domain_balanced_action_v126"
VATD_MAP50 = 0.93844
...
def blend(scores: list[np.ndarray], weights: tuple[float, float, float], mode: str) -> np.ndarray:
...
def load_final_scores(split: str, data: dict[str, Any], locations: list[tuple[str, int, int, str, float]]) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    v126_summary = json.loads((V126 / "official_summary.json").read_text(encoding="utf-8"))
    selected_ratio = float(v126_summary["selected_ratio"])
    selected_record = next(record for record in v126_summary["ratios"] if abs(float(record["ratio"]) - selected_ratio) < 1e-9)
    v126_config = v126_summary["validation_selection"]
    v126_path = Path(selected_record["val_scores"] if split == "val" else selected_record["test_scores"])
    v126_map, _ = load_row_scores(v126_path, str(selected_record["field"]), 1)
    v129_summary = json.loads((V129 / "official_summary.json").read_text(encoding="utf-8"))
    selected_ratio129 = float(v129_summary["selected_ratio"])
    selected_record129 = next(record for record in v129_summary["ratios"] if abs(float(record["ratio"]) - selected_ratio129) < 1e-9)
    v129_config = v129_summary["validation_selection"]
    v129_path = Path(selected_record129["val_scores"] if split == "val" else selected_record129["test_scores"])
    v129_map, _ = load_row_scores(v129_path, str(selected_record129["field"]), 1)

    scores53: list[float] = []
    scores126: list[float] = []
    scores129: list[float] = []
    for sequence, frame_id, index, image_id, raw in locations:
        ...
        scores126.append(fuse_score(raw, float(v126_map.get(key, raw)), float(v126_config["alpha"]), str(v126_config["mode"])))
        scores129.append(fuse_score(raw, float(v129_map.get(key, raw)), float(v129_config["alpha"]), str(v129_config["mode"])))
    return np.asarray(scores53), np.asarray(scores126), np.asarray(scores129)


def main() -> int:
    OUT.mkdir(parents=True, exist_ok=True)
    report("wait_models", 0)
    wait_for(V126 / "official_summary.json", UPSTREAM_RUN)
    ...
    for mode in ("logit", "geom", "linear"):
        for weight126 in grid:
            for weight129 in grid:
                if weight126 + weight129 > 1.000001:
                    continue
                weights = (1.0 - weight126 - weight129, weight126, weight129)
                result = {"mode": mode, "weights": {"v53": weights[0], "v126": weights[1], "v129": weights[2]}, **metrics(correct, blend(val_scores, weights, mode), pred_cls, target_cls, Path(r"D:\urap_modal_stage\TransVisDrone"))}
                rows.append(result)
    ...
    weights = (float(best["weights"]["v53"]), float(best["weights"]["v126"]), float(best["weights"]["v129"]))
