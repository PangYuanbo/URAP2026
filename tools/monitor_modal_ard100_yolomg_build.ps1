...
$lines = if ($stdout) { @(Get-Content -LiteralPath $stdout.FullName -ErrorAction SilentlyContinue) } else { @() }
$completedVideos = @($lines | Select-String '\[SPLIT=(train|val|test)\] done video=').Count
$expectedVideos = 100
$lastVideo = $lines | Select-String '\[SPLIT=(train|val|test)\] done video=' | Select-Object -Last 1
Write-Host "done/total=$completedVideos/$expectedVideos"
if ($lastVideo) { Write-Host "last completed unit: $($lastVideo.Line)" }
if ($stdout) { Write-Host "stdout log: $($stdout.FullName) updated=$($stdout.LastWriteTime.ToString('o'))"; Get-Content -LiteralPath $stdout.FullName -Tail 12 }
if ($stderr) { Write-Host "stderr log: $($stderr.FullName) updated=$($stderr.LastWriteTime.ToString('o'))"; Get-Content -LiteralPath $stderr.FullName -Tail 12 }
