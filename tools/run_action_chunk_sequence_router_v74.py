...
REPO=Path(r'C:\Users\aaron\Desktop\URAP');RUN=REPO/'artifacts'/'detached_action_chunk_sequence_router_v74';PROGRESS=RUN/'progress.json';OUT=Path(r'D:\URAP_vatd_rank_results\action_chunk_sequence_router_v74');V46=Path(r'D:\URAP_vatd_rank_results\action_chunk_neighbor_model_v46');V51=Path(r'D:\URAP_vatd_rank_results\action_chunk_candidate_context_v51');V52=Path(r'D:\URAP_vatd_rank_results\action_chunk_multi_expert_v52');FPS=REPO/'data_templates'/'nps_sequence_fps.json'
...
def main():
 OUT.mkdir(parents=True,exist_ok=True);tool=REPO/'tools'/'sweep_action_chunk_sequence_router.py';common=[sys.executable,str(tool),'--tvd-root',r'D:\urap_modal_stage\TransVisDrone','--fps-json',str(FPS)]
 execute('select_sequence_router',0,common+['--predictionsgt-pkl',r'D:\URAP_nps_val_tvd\runs\nps_val_rank_source\predictionsgt\predictionsgt_split_0_official_labels.pkl','--v46',str(V46/'val_oof_scores.jsonl'),'--v51',str(V51/'val_oof_scores.jsonl'),'--v52',str(V52/'val_expert_scores.jsonl'),'--out-json',str(OUT/'val_sweep.json')])
 ...
 validation=json.loads((OUT/'val_sweep.json').read_text(encoding='utf8'))['best'];test=json.loads((OUT/'test_fixed.json').read_text(encoding='utf8'))['best'];summary={'protocol':'pure Action Chunk offline sequence routing by label-free persistent-action fraction; OOF selection; fixed test','validation_selection':validation,'test_fixed':test,'target_map50':.97,'target_met':test['map50']>=.97};(OUT/'official_summary.json').write_text(json.dumps(summary,indent=2),encoding='utf8');report('done',2,summary=summary);return 0
