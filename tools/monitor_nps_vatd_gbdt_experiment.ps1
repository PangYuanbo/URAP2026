...
$parent = Split-Path $OutputRoot -Parent; $summary = Join-Path $parent 'nps_vatd_gbdt_trainval_v1\fusion_sweep.json'
if (Test-Path $summary) { $data = Get-Content $summary -Raw | ConvertFrom-Json; Write-Host "last_completed_unit=evaluation best_map50=$($data.best.map50) best_recall=$($data.best.recall)" } elseif (Test-Path (Join-Path $parent 'nps_vatd_gbdt_trainval_v1\train_summary.json')) { Write-Host 'last_completed_unit=train' } else { Write-Host 'last_completed_unit=none' }
